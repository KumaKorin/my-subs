...
function setCurrentProfileId(id2) {
  currentProfileId = id2;
  notifyListeners();
}
...
function parseTrafficInfo(headerStr) {
  ...
  try {
    ...
    for (const p of parts) {
      const [k, v] = p.split("=");
      if (k && v !== void 0) info[k.trim()] = parseInt(v.trim(), 10) || 0;
    }
    const used = (info.upload || 0) + (info.download || 0);
    const total = info.total || 0;
    let expireDate = "\u65E0\u9650\u5236";
    if (info.expire && info.expire > 0) {
      expireDate = new Date(info.expire * 1e3).toLocaleDateString();
    }
    return {
...
function applyDOMChange(view, domChange) {
  let change;
  let { newSel } = domChange, { state: state2 } = view, sel = state2.selection.main;
  ...
  if (domChange.bounds) {
    ...
    let cmp = state2.doc.sliceString(from, to, LineBreakPlaceholder), selEnd, diff;
  ...
  } else if (newSel && (!view.hasFocus && state2.facet(editable) || sameSelPos(newSel, sel))) {
    newSel = null;
  }
  if (!change && !newSel)
    return false;
  ...
  } else if (state2.doc.lineAt(sel.from).to < sel.to && view.docView.lineHasWidget(sel.to) && view.inputState.insertingTextAt > Date.now() - 50) {
    change = {
      from: sel.from,
      to: sel.to,
      insert: state2.toText(view.inputState.insertingText)
    };
  ...
  if (change) {
    ...
    if (view.inputState.lastSelectionTime > Date.now() - 50) {
      ...
      if (userEvent == "select.pointer")
        newSel = skipAtomsForSelection(state2.facet(atomicRanges).map((f) => f(view)), newSel);
    }
...
var ViewState = class {
  constructor(view, state2) {
    ...
    this.stateDeco = staticDeco(state2);
...
function staticDeco(state2) {
  let deco = state2.facet(decorations).filter((d) => typeof d != "function");
  let outer = state2.facet(outerDecorations).filter((d) => typeof d != "function");
  if (outer.length)
...
var ParseContext = class _ParseContext {
  constructor(parser3, state2, fragments = [], tree, treeLen, viewport, skipped, scheduleOn) {
    this.parser = parser3;
    this.state = state2;
  ...
  @internal
  */
  static create(parser3, state2, viewport) {
    return new _ParseContext(parser3, state2, [], Tree.empty, 0, viewport, [], null);
  }
  startParse() {
...
var historyField_ = /* @__PURE__ */ StateField.define({
  ...
  update(state2, tr) {
    ...
    if (event)
      state2 = state2.addChanges(event, time, userEvent, config2, tr);
...
var HistoryState = class _HistoryState {
  ...
  pop(side, state2, onlySelection) {
    ...
    if (branch.length == 0)
      return null;
    let event = branch[branch.length - 1], selection2 = event.selectionsAfter[0] || (event.startSelection ? event.startSelection.map(event.changes.invertedDesc, 1) : state2.selection);
...
var gotoLine = (view) => {
  ...
  let line = String(state2.doc.lineAt(view.state.selection.main.head).number);
  let { close, result } = showDialog(view, {
    label: state2.phrase("Go to line"),
    input: { type: "text", name: "line", value: line },
    focus: true,
    submitLabel: state2.phrase("go")
  });
  result.then((form) => {
...
var matchHighlighter = /* @__PURE__ */ ViewPlugin.fromClass(class {
  ...
  getDeco(view) {
    ...
    for (let part of view.visibleRanges) {
      let cursor2 = new SearchCursor(state2.doc, query, part.from, part.to);
...
function findNextOccurrence(state2, query) {
  ...
  for (let cycled = false, cursor2 = new SearchCursor(state2.doc, query, ranges[ranges.length - 1].to); ; ) {
    cursor2.next();
    if (cursor2.done) {
      if (cycled)
        return null;
      cursor2 = new SearchCursor(state2.doc, query, 0, Math.max(0, ranges[ranges.length - 1].from - 1));
      cycled = true;
    } else {
      if (cycled && ranges.some((r) => r.from == cursor2.value.from))
        continue;
      if (fullWord) {
        let word2 = state2.wordAt(cursor2.value.from);
...
function wrapStringTest(test, state2, inner) {
  return (from, to, buffer, bufferPos) => {
    if (inner && !inner(from, to, buffer, bufferPos))
      return false;
    let match = from >= bufferPos && to <= bufferPos + buffer.length ? buffer.slice(from - bufferPos, to - bufferPos) : state2.doc.sliceString(from, to);
    return test(match, state2, from, to);
  };
}
...
var StringQuery = class extends QueryType2 {
  ...
  nextMatch(state2, curFrom, curTo) {
    let cursor2 = stringCursor(this.spec, state2, curTo, state2.doc.length).nextOverlapping();
    if (cursor2.done) {
      let end = Math.min(state2.doc.length, curFrom + this.spec.unquoted.length);
      cursor2 = stringCursor(this.spec, state2, 0, end).nextOverlapping();
    }
  ...
  prevMatchInRange(state2, from, to) {
    for (let pos = to; ; ) {
      ...
      let cursor2 = stringCursor(this.spec, state2, start, pos), range = null;
  ...
  matchAll(state2, limit) {
    let cursor2 = stringCursor(this.spec, state2, 0, state2.doc.length), ranges = [];
    while (!cursor2.next().done) {
  ...
  highlight(state2, from, to, add2) {
    let cursor2 = stringCursor(this.spec, state2, Math.max(0, from - this.spec.unquoted.length), Math.min(to + this.spec.unquoted.length, state2.doc.length));
    while (!cursor2.next().done)
...
function wrapRegexpTest(test, state2, inner) {
  return (from, to, match) => {
    return (!inner || inner(from, to, match)) && test(match[0], state2, from, to);
  };
}
function regexpCursor(spec, state2, from, to) {
  let test;
  if (spec.wholeWord)
    test = regexpWordTest(state2.charCategorizer(state2.selection.main.head));
  if (spec.test)
...
var RegExpQuery = class extends QueryType2 {
  nextMatch(state2, curFrom, curTo) {
    let cursor2 = regexpCursor(this.spec, state2, curTo, state2.doc.length).next();
    if (cursor2.done)
      cursor2 = regexpCursor(this.spec, state2, 0, curFrom).next();
  ...
  prevMatchInRange(state2, from, to) {
    for (let size = 1; ; size++) {
      ...
      let cursor2 = regexpCursor(this.spec, state2, start, to), range = null;
      while (!cursor2.next().done)
        range = cursor2.value;
  ...
  matchAll(state2, limit) {
    let cursor2 = regexpCursor(this.spec, state2, 0, state2.doc.length), ranges = [];
    while (!cursor2.next().done) {
      if (ranges.length >= limit)
        return null;
      ranges.push(cursor2.value);
  ...
  highlight(state2, from, to, add2) {
    let cursor2 = regexpCursor(this.spec, state2, Math.max(
      0,
      from - 250
...
var CompletionDialog = class _CompletionDialog {
  ...
  static build(active, state2, id2, prev, conf, didSetActive) {
...
var ActiveSource = class _ActiveSource {
  constructor(source, state2, explicit = false) {
    this.source = source;
    this.state = state2;
    this.explicit = explicit;
  }
...
var completionPlugin = /* @__PURE__ */ ViewPlugin.fromClass(class {
  ...
  startQuery(active) {
    ...
    let context = new CompletionContext(state2, pos, active.explicit, this.view);
...
function handleSame(state2, token, allowTriple, config2) {
  ...
  let dont = null, changes = state2.changeByRange((range) => {
    ...
    } else if (allowTriple && state2.sliceDoc(pos - 2 * token.length, pos) == token + token && (start = canStartStringAt(state2, pos - 2 * token.length, stringPrefixes)) > -1 && nodeStart(state2, start)) {
      return {
...
function canStartStringAt(state2, pos, prefixes) {
  ...
  for (let prefix of prefixes) {
    ...
    if (state2.sliceDoc(start, pos) == prefix && charCat(state2.sliceDoc(start - 1, start)) != CharCategory.Word)
      return start;
  }
  return -1;
}
...
var LintState = class _LintState {
  ...
  static init(diagnostics, panel, state2) {
    ...
    if (diagnosticFilter)
      diagnostics = diagnosticFilter(diagnostics, state2);
    let sorted = diagnostics.slice().sort((a, b) => a.from - b.from || a.to - b.to);
    let deco = new RangeSetBuilder(), active = [], pos = 0;
    let scan = state2.doc.iter(), scanPos = 0, docLen = state2.doc.length;
    for (let i2 = 0; ; ) {
...
var Stack = class _Stack {
  ...
  @internal
  */
  constructor(p, stack, state2, reducePos, pos, score2, buffer, bufferBase, curContext, lookAhead = 0, parent) {
    this.p = p;
    this.stack = stack;
    this.state = state2;
  ...
  findForcedReduction() {
    ...
    let explore = (state2, depth) => {
      if (seen.includes(state2))
        return;
      seen.push(state2);
      return parser3.allActions(state2, (action) => {
...
var TokenCache = class {
  ...
  addActions(stack, token, end, index) {
    let { state: state2 } = stack, { parser: parser3 } = stack.p, { data } = parser3;
  ...
  parser: /* @__PURE__ */ parser.configure({
    props: [
      ...
      /* @__PURE__ */ foldNodeProp.add({
        ...
        "Item Pair BlockLiteral": (node, state2) => ({ from: state2.doc.lineAt(node.from).to, to: node.to })
      })
    ]
  }),
  languageData: {
...
function createYamlLinter(statusElementId) {
  return linter((view) => {
    ...
    try {
      ...
      if (statusEl) {
        statusEl.style.display = "inline-flex";
        const lineNum = e.mark?.line !== void 0 ? e.mark.line + 1 : "?";
...
function createEditor(container, doc2 = "", readOnly2 = false, linterStatusId = null) {
...
function setEditorContent(editorView, content2) {
...
var ProvidersComponent = class {
  ...
  addProvider() {
    this.providers.push({
      id: crypto.randomUUID ? crypto.randomUUID() : `prov_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`,
      name: `Provider_${this.providers.length + 1}`,
      type: "http",
      interval: 36e3,
      healthCheckEnable: true,
      healthCheckInterval: 36e3,
      proxy: "DIRECT",
      url: "",
  ...
  updateProvider(index, field, value) {
    if (field === "interval" || field === "healthCheckInterval") {
      this.providers[index][field] = parseInt(value, 10) || 36e3;
    } else if (field === "healthCheckEnable" || field === "useWorkerProxy" || field === "useFetchProxy" || field === "proxyRedirect") {
  ...
  render() {
    ...
    const html = this.providers.map((p, idx) => {
                    ...
                    <div class="provider-item" data-index="${idx}">
                        ...
                        <div class="provider-row">
                            <div class="field-group">
                                ...
                                <input type="number" class="input-field provider-interval" value="${p.interval || 36e3}">
                            </div>
                            <div class="field-group">
  ...
  bindEvents() {
    this.container.querySelectorAll(".provider-item").forEach((itemEl) => {
      ...
      itemEl.querySelector(".provider-use-fetch-proxy")?.addEventListener("change", (e) => {
        this.updateProvider(idx, "useFetchProxy", e.target.value);
        this.render();
      });
      itemEl.querySelector(".provider-fetch-proxy-url")?.addEventListener("input", (e) => {
        this.updateProvider(idx, "fetchProxyUrl", e.target.value);
      });
      itemEl.querySelector(".provider-proxy-redirect")?.addEventListener("change", (e) => {
...
var customYamlEditorView = null;
var previewEditorView = null;
function renderProfileSelect() {
  ...
  select.innerHTML = (profiles || []).map(
    (p) => `<option value="${p.id}" ${p.id === currentId ? "selected" : ""}>${p.name || "\u672A\u547D\u540D Profile"}</option>`
  ).join("");
}
...
function renderProfileProvidersList(profile) {
  ...
  if (pool.length === 0) {
    ...
    if (btn) {
      btn.addEventListener("click", () => {
        const provTab = document.querySelector('.nav-tab[data-tab="tab-providers"]');
  ...
  container.innerHTML = pool.map((p) => {
    ...
    let statusDot = "";
    if (p.lastStatus) {
      statusDot = p.lastStatus >= 200 && p.lastStatus < 300 ? '<span class="status-dot-ok" title="200 OK"></span>' : '<span class="status-dot-err" title="\u5F02\u5E38"></span>';
    }
    return `
  ...
  container.querySelectorAll(".profile-provider-checkbox-item").forEach((el) => {
    ...
    toggle.addEventListener("change", (e) => {
      ...
      if (e.target.checked) {
        ...
        profile.enabledProviderIds = profile.enabledProviderIds.filter((id2) => id2 !== provId);
      }
    });
  });
}
...
function bindProfileEvents() {
  ...
  if (btnNewProfile) {
    btnNewProfile.addEventListener("click", () => {
      ...
      const name2 = prompt("\u8BF7\u8F93\u5165\u65B0 Profile \u540D\u79F0:", `Profile_${(profiles || []).length + 1}`);
    ...
    btnSaveCurrentProfile.addEventListener("click", async () => {
      ...
      if (!profile.useGlobalYaml && window.jsyaml && profile.customBaseYaml?.trim()) {
        try {
          ...
          const lineInfo = err.mark?.line !== void 0 ? ` (\u7B2C ${err.mark.line + 1} \u884C)` : "";
...
async function loadLogs(page = null) {
  ...
  try {
    ...
    tbody.innerHTML = logs.map((log) => {
      ...
      const durationStr = log.duration_ms !== null && log.duration_ms !== void 0 ? `<span class="${durationClass}">${log.duration_ms} ms</span>` : "-";
...
function startLogsPolling() {
  refreshLogsAndStats();
  if (!logsTimer) {
    logsTimer = setInterval(refreshLogsAndStats, 1e4);
  }
}
...
// src/client/app.ts
var poolProvidersComp = null;
var globalYamlEditorView = null;
...
async function initApp() {
  try {
    const resData = await apiRequest("/api/data");
...
function switchTab(tabId) {
  document.querySelectorAll(".nav-tab").forEach((t2) => {
    if (t2.getAttribute("data-tab") === tabId) {
...
function bindGlobalEvents() {
  const basePrefix2 = getBasePrefix();
  const btnThemeToggle = document.getElementById("btn-theme-toggle");
  const themeToggleIcon = document.getElementById("theme-toggle-icon");
  function updateThemeIcon(theme2) {
    if (themeToggleIcon) {
      themeToggleIcon.className = theme2 === "light" ? "ri-moon-line" : "ri-sun-line";
    }
  }
  ...
  if (btnLogout) {
    btnLogout.addEventListener("click", async () => {
      ...
      setTimeout(() => {
        window.location.href = `${basePrefix2}/login`;
      }, 300);
    });
  }
  ...
  if (btnSavePool) {
    btnSavePool.addEventListener("click", async () => {
      ...
      const result = await apiRequest("/api/providers-pool", {
        method: "POST",
        body: { providers }
      });
      if (result.success) {
        setState({ providersPool: result.providers || providers });
  ...
  if (btnSaveGlobalYaml) {
    btnSaveGlobalYaml.addEventListener("click", async () => {
      const yaml2 = globalYamlEditorView ? globalYamlEditorView.state.doc.toString() : "";
      if (window.jsyaml && yaml2.trim()) {
        try {
          window.jsyaml.load(yaml2);
        } catch (err) {
          const lineInfo = err.mark?.line !== void 0 ? ` (\u7B2C ${err.mark.line + 1} \u884C)` : "";
          const confirmSave = confirm(
